...
Interactive clinical decision support tool for **fever of unknown origin** workup. Single-file HTML — no build step, no server, no dependencies. Drop it on GitHub Pages or open it directly in a browser.

---

## What it is

A reactive, exposure-driven differential generator for FUO. You check off patient features (geography, travel, exposures, symptoms, lab clues) and the tool scores and ranks 52 diseases in real time, shows you *why* each diagnosis is on the list, fires red-flag alerts with empiric treatment recommendations, and outputs a copyable case summary.

Tuned for an **Oklahoma clinical context** — RMSF, ehrlichiosis, tularemia, brucellosis, Q fever, histoplasmosis, and blastomycosis are weighted accordingly — while covering global traveler diseases and common non-infectious causes.

---

## What's in it

**52 diseases across 4 categories**

| Category | Count | Examples |
|---|---|---|
| Infection | 26 | TB, endocarditis, RMSF, malaria, histoplasmosis, EBV, typhoid |
| Malignancy | 9 | Hodgkin/NHL, leukemia, RCC, HCC, myeloma, atrial myxoma |
| Inflammatory | 10 | Still's, GCA, SLE, ANCA vasculitis, sarcoid, FMF, Behçet |
| Misc | 7 | Drug fever, VTE, IBD, factitious fever, periodic fever syndromes |

**Panels and features**

- **Reactive DDx** — diseases score from weighted triggers (exposures, symptoms, labs). The list re-ranks on every chip click. Match tags show exactly which inputs moved a disease up the list.
- **Red-flag strip** — fires on combined criteria, not individual flags. Examples: tick exposure + Oklahoma + thrombocytopenia/transaminitis → empiric doxycycline prompt; age ≥50 + new headache/vision change + elevated ESR → empiric steroids for GCA; any endemic travel → rule out malaria first.
...
- **Case summary generator** — outputs a plain-text block with FUO type, demographics, inputs, top ranked differential with scores, and always-consider list. Good for sign-out or attending presentations.
...
Everything is in one `<script>` block. No framework, no build tooling.
...
The four `Set` objects hold the active checkbox values. Every input change calls `render()`.
...
```js
{
  ...
  triggers: [
    ['tick', 6], ['rash_wrists_ankles', 6], ['thrombocytopenia', 3],
    ['transaminitis', 3], ['hyponatremia', 3], ['new_headache', 2], ['myalgia', 2]
  ],
  ...
}
```

`scoreDisease()` iterates triggers, checks all four state Sets, and sums the weights. The DDx list sorts by score descending. `alwaysConsider: true` diseases appear at the end of the list even at score 0.

### Red flags

`renderRedFlags()` checks combined conditions against `state` directly — it doesn't use the scoring system. Each condition is a logical `&&` / `||` check that produces a colored alert strip with specific empiric treatment text.

---

## How to add a disease

Add an object to the `DISEASES` array in the script. The required fields:
...
```js
{
  ...
  regional: true,            // shows "OK regional" badge
...
Keep the total achievable score in mind when adding triggers — a disease with 10 triggers of weight 2 will rank comparably to one with 2 triggers of weight 5, which is often not the intended behavior.

---

## How to add a new input chip

1. Add the `<label class="chip">` element to the appropriate `<details>` section in the HTML, with a unique `value`:
   ```html
   <label class="chip"><input type="checkbox" value="my_new_field"><span>Display label</span></label>
   ```
2. Add a human-readable label to the `LABELS` object in the script:
   ```js
   my_new_field: 'display label',
   ```
3. Reference `my_new_field` in any disease's `triggers` array.

The chip will be picked up automatically by the `data-multi` binding — no additional JS needed.

---

## How to add a red flag

Add a block inside `renderRedFlags()` using the same pattern as the existing ones:
...
Red flags intentionally require *combined* criteria (not a single trigger) to avoid alert fatigue.

---

## Deployment

The file is self-contained. Options:

- **GitHub Pages** — commit `fuo-workup.html` to any Pages-enabled repo. It renders immediately.
- **Local** — open the file directly in a browser. No server needed.
- **Embedded** — the file can be iframed into any existing page without modification.

There is no state persistence by design — no cookies, no localStorage, no server calls. Safe for clinical environments where no PHI should be stored.
